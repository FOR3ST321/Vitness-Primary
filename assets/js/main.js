function validate(fields) {
    ...
    if(fields.name === 'phone'){
        if(!fields.value.startsWith('+81') && (fields.value.substr(3).length < 11)){
            data.isValid = false;
            data.errorMsg.push({
                field : 'phone',
    ...
    if(fields.name === 'email'){
        if(!fields.value.endsWith(".com") && !(fields.value.startsWith('@')) && (fields.value.split('@').length != 1)){
            data.isValid = false;
            data.errorMsg.push({
                field : 'email',
...
$("#joinForm").submit((e) => {
    ...
    $.each(data, function (indexInArray, valueOfElement) {
        console.log(validate(valueOfElement));
    });
});
